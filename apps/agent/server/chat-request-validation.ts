...
const CLEAR_CHAT_BODY_ALLOWED_KEYS = new Set(['conversationId']);
...
const FEEDBACK_RATING_VALUES = ['up', 'down'] as const;
const MAX_FEEDBACK_TEXT_LENGTH = 4000;
...
function hasDisallowedFeedbackControlCharacters(value: string): boolean {
  return Array.from(value).some((char) => {
    const code = char.charCodeAt(0);
...
/**
 * Validates POST /chat/clear body. Only allows conversationId.
 */
export function validateClearChatBody(body: unknown): ValidateClearChatBodyResult {
  if (!isRecord(body)) {
    return { ok: false, status: 400, error: 'Request body must be a JSON object.' };
  }
  const unknownKeys = Object.keys(body).filter((key) => !CLEAR_CHAT_BODY_ALLOWED_KEYS.has(key));
  if (unknownKeys.length > 0) {
    return {
      ok: false,
      status: 400,
...
export function parseFeedbackBody(value: unknown):
  | { ok: true; params: ParsedFeedbackBody }
  | { ok: false; status: 400; error: string } {
  if (!isRecord(value)) {
    return { ok: false, status: 400, error: 'Request body must be a JSON object.' };
  }
  ...
  if (value.correction !== undefined && correction === undefined) {
    return { ok: false, status: 400, error: 'correction must be a non-empty string when provided.' };
  }
